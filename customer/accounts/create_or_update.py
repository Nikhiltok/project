...
class CreateOrUpdate(GenericAPIView):
	...
	def UserCreateOrUpdate(self, data):
		...
		if data.get('id'):
			...
			username1 = str(data.get("email")).lower()
			instance = UserProfile.objects.filter(
		...
		if not instance:
			...
			user.is_active=True
			instance.terms_condition_privacy = data.get('terms_condition_privacy')

			# check otp send or not
			# sms_flag = False
			# if store_ins.is_sms_verification:
			# 	# send otp here
			# 	send_sms(phone,store_id)
		else:
